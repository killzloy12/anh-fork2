#!/usr/bin/env python3
"""
🧠 HANDLERS v3.2 - ИСПРАВЛЕННАЯ ВЕРСИЯ ДЛЯ ПЕРСОНАЖЕЙ ТОЛЬКО В ГРУППАХ
🚀 Все ошибки исправлены, система персонажей v3.2
"""
...
def register_all_handlers(dp, modules):
    ...
    router = Router()
    bot_info = None  # ← Добавьте эту строку
    
    # Импорт интеграции AI-персонажей
    from app.handlers.personality_ai_integration_v3_2 import integrate_personality_ai_logic
    from app.handlers.personality_handlers_v3_2 import register_personality_handlers

    # Регистрируем персонажей
    register_personality_handlers(router, modules)
    # Регистрируем AI-логику персонажей
    integrate_personality_ai_logic(router, modules)
    
    async def get_bot_info():
        nonlocal bot_info  # Теперь это будет работать
        try:
    ...
    async def cmd_be_personality(message: Message):
        ...
        try:
            # Проверяем права доступа через новую систему v3.2
            ...
            if not can_use:
                ...
                if message.chat.id < 0:  # Группа
                    ...
                    await message.reply(
                        ...
                        f"2. Попросите админа бота установить персонажа\n"
                        f"3. Наслаждайтесь общением с персонажем!\n\n"
                        f"💡 **Зачем так:** Персонажи созданы для развлечения участников групп"
                    )
                return
        ...
        try:
            ...
            if not can_use:
                ...
                if message.chat.id < 0:  # Группа
                    await message.reply(
                        f"🚫 **Доступ запрещен**\n\n"
                        f"{access_reason}\n\n"
                        f"👑 **Только админы бота могут сбрасывать персонажи в группах**"
                    )
                else:  # Личный чат
                    await message.reply(
                        f"🚫 **Система персонажей недоступна**\n\n"
                        f"📍 **Персонажи работают только в групповых чатах**\n\n"
                        f"ℹ️ Используйте эту команду в группе где установлен персонаж"
                    )
                return
            ...
            if not active_personality:
                ...
                await message.reply(
                    ...
                    f"Для установки персонажа используйте:\n"
                    f"`/be описание персонажа`"
                )
                return
    ...
    async def cmd_current_persona(message: Message):
        ...
        try:
            ...
            chat_info = await personality_manager.get_chat_personality_info(message.chat.id)
            ...
            if message.chat.id < 0:  # Группа
                ...
                response += f"👑 **Создатель:** ID {chat_info['admin_id'] if chat_info else 'неизвестно'}\n"
                response += f"📅 **Установлен:** {chat_info['created_at'][:10] if chat_info else 'неизвестно'}\n\n"
                response += f"🎯 **Охват:** Все участники группы\n"
                response += f"⚡ **Управление:** Только админы бота"
            else:  # ЛС админа
                response += f"🧪 **Тип:** Тестовый режим\n"
                response += f"👑 **Админ:** {message.from_user.first_name}\n"
    ...
    logger.info("🎛️ Все обработчики v3.2 зарегистрированы (персонажи только в группах)")